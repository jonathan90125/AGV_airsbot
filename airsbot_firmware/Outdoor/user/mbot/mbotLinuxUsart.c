...
}odx,ody,oda,imu1,imu2,imu3,imu4,imu5,imu6,imu7,imu8,imu9,power1;
...
int usartReceiveOneData(int *p_leftSpeedSet,int *p_rightSpeedSet,unsigned char *p_crtlFlag)
{
	...
	USART_Receiver = USART_ReceiveData(UART4);   //@@@@@#####如果你使用不是USART1更改成相应的，比如USART3
	//接收消息头
	if(Start_Flag == START)
	{
...
void usartSendData(short ox0, short oy0,short oa0,short imu_1,short imu_2,short imu_3,short imu_4,short imu_5,short imu_6,short imu_7,short imu_8,short imu_9,short power_1, unsigned char ult_byte)
{
	...
	for(i = 0; i < 2; i++)
	{
		buf[i + 3] = odx.data[i];         // buf[3] buf[4]
		buf[i + 5] = ody.data[i];        // buf[5] buf[6]
		buf[i + 7] = oda.data[i];           // buf[7] buf[8]
		buf[i + 9] = imu1.data[i];         // buf[3] buf[4]
		buf[i + 11] =imu2.data[i];        // buf[5] buf[6]
		buf[i + 13] =imu3.data[i];           // buf[7] buf[8]
		buf[i + 15] =imu4.data[i];         // buf[3] buf[4]
		buf[i + 17] =imu5.data[i];        // buf[5] buf[6]
		buf[i + 19] =imu6.data[i];           // buf[7] buf[8]
		buf[i + 21] =imu7.data[i];         // buf[3] buf[4]
    buf[i + 23] =imu8.data[i];           // buf[7] buf[8]
		buf[i + 25] =imu9.data[i];
    buf[i + 27] =power1.data[i];		
	}
	// 预留控制指令
	buf[3 + length - 1] = ult_byte;              // buf[9]
...
static void crc16_update(uint16_t *currectCrc, const uint8_t *src, uint32_t lengthInBytes)
	{ 
		uint32_t crc = *currectCrc; uint32_t j; 
