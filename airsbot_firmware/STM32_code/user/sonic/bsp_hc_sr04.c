...
static void ultrasonic_extiConfig(void)
{
  ...
  GPIO_EXTILineConfig(UL_GPIO_PORT_SOURCE, GPIO_PinSource14 ); //将中断线连接到GPIO端口
...
{		
  /*Ultrasonic 1*/	
    ...
    EXTI_ClearITPendingBit(EXTI_Line14);  
		...
		TIM_Cmd(TIM3, ENABLE);
		while(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_14))  //上升沿触发中断后等待变为低电平
    {
    ...
    EXTI_ClearITPendingBit(EXTI_Line15);  
		TIM_SetCounter(TIM3, 0);
		TIM_Cmd(TIM3, ENABLE);
		while(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_15))  //上升沿触发中断后等待变为低电平
    {
      if(TIM_GetCounter(TIM3) >= UL_TIM_MAX_COUNT) // cnt = 255cm * 2 /(340 * 100) * 100000
      {
        break;  //超出设定的测量最大距离，跳出等待
